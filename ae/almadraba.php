...
// Crear estructura para actividades populares
$actividadesPopulares = [
    'Infantil' => [],
    'Primaria' => []
];

// Filtrar actividades populares (cupo_actual > 7)
foreach ($actividadesCentro as $actividad) {
    if ($actividad['cupo_actual'] > 7) {
        if ($actividad['curso_maximo'] <= 3) {
            $actividadesPopulares['Infantil'][] = $actividad;
        } else {
            $actividadesPopulares['Primaria'][] = $actividad;
        }
    }
}

// Ordenar actividades populares por nombre
foreach ($actividadesPopulares as &$actividades) {
    usort($actividades, function($a, $b) {
        return strcmp($a['actividad'], $b['actividad']);
    });
}
...
<div class="container-fluid bg-light py-5">
    <div class="container">
        ...
        <!-- Modificar la sección de actividades -->
        <section id="actividades">
            <!-- Tabs de navegación -->
            <ul class="nav nav-tabs mb-4" id="actividadesTab" role="tablist">
                ...
                <li class="nav-item" role="presentation">
                    <button class="nav-link" id="populares-tab" data-bs-toggle="tab" data-bs-target="#populares" type="button" role="tab" aria-controls="populares" aria-selected="false">
            ...
            <div class="tab-content" id="actividadesTabContent">
                                    ...
                                    <?php foreach ($actividades as $actividad): ?>
                                        <div class="p-3 border-bottom">
                                            ...
                                            <div class="small text-secondary mb-1">
                                                ...
                                                <?= getNombreCurso($actividad['curso_minimo']) ?> - <?= getNombreCurso($actividad['curso_maximo']) ?>
                                            </div>
                                            <div class="small text-secondary mb-1">
                                        ...
                                        <tbody>
                                            <?php foreach ($actividades as $actividad): ?>
                                                <tr>
                                                    ...
                                                    <td><?= getNombreCurso($actividad['curso_minimo']) ?> - <?= getNombreCurso($actividad['curso_maximo']) ?></td>
                ...
                <!-- Tab: Actividades populares -->
                <div class="tab-pane fade" id="populares" role="tabpanel" aria-labelledby="populares-tab">
                    <?php if (empty(array_filter($actividadesPopulares))): ?>
                        <div class="alert alert-info">
                            No hay actividades con alta demanda en este momento.
                        </div>
                    <?php else: ?>
                        <?php foreach ($actividadesPopulares as $nivel => $actividades): ?>
